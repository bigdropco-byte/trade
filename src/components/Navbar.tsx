...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200/80 shadow-xs">
      ...
      {/* Top Banner / Privacy Bar */}
      <div className="bg-slate-50 border-b border-slate-200 px-3 sm:px-4 py-1 text-xs flex items-center justify-between text-slate-600 gap-2 overflow-x-auto no-scrollbar">
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          <button 
            onClick={onOpenPrivacy}
            className="flex items-center gap-1.5 text-emerald-800 hover:text-emerald-900 transition-colors font-medium cursor-pointer shrink-0"
          >
            <ShieldCheck className="w-3.5 h-3.5 text-emerald-700 shrink-0" />
            <span className="hidden sm:inline">100% Client-Side Private — Zero Server Storage</span>
            <span className="sm:hidden text-[11px]">100% Client-Side Private</span>
          </button>
          ...
          {onTogglePrivacyMask && (
            ...
            >
              {isPrivacyMasked ? <EyeOff className="w-3 h-3" /> : <Eye className="w-3 h-3 text-slate-600" />}
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            ...
            className="flex items-center gap-1 text-[11px] sm:text-xs text-slate-700 hover:text-slate-900 font-medium transition-colors cursor-pointer mr-1 sm:mr-2"
          ...
          <button
            onClick={onLoadDemo}
            className="flex items-center gap-1 text-[11px] sm:text-xs text-indigo-700 hover:text-indigo-800 font-medium transition-colors cursor-pointer"
            title="Reset to Sample Journal"
          >
            <RefreshCw className="w-3 h-3" />
            <span className="hidden sm:inline">Sample Journal</span>
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 gap-2 sm:gap-4">
          ...
          <div 
            ...
            title="Return to Homepage"
          >
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-gradient-to-tr from-emerald-600 via-teal-600 to-indigo-600 p-0.5 shadow-md shadow-emerald-600/10 group-hover:scale-105 transition-transform shrink-0">
              <div className="w-full h-full bg-white rounded-[9px] sm:rounded-[10px] flex items-center justify-center">
                <Activity className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-700" />
              </div>
            </div>
            <div className="min-w-0">
              <div className="flex items-center gap-1.5 sm:gap-2">
                <span className="text-base sm:text-xl font-extrabold tracking-tight text-slate-900 group-hover:text-emerald-700 transition-colors truncate">
              ...
              <p className="text-[11px] text-slate-600 hidden sm:block">Zero-Knowledge Trading Journal & Scrapbook</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 sm:px-3.5 py-1.5 sm:py-2 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-slate-950 font-bold text-xs rounded-xl shadow-xs hover:shadow-sm transition-all cursor-pointer shrink-0"
            >
            ...
            <button
              onClick={onExportPdf}
              className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 sm:py-2 bg-white hover:bg-slate-100 text-slate-700 hover:text-slate-900 font-medium text-xs rounded-xl border border-slate-200 shadow-xs transition-colors cursor-pointer shrink-0"
